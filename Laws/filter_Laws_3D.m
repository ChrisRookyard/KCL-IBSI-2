function imageFiltered = filter_Laws_3D...
    (image,lawsKernel,padType,invariant,poolingType,energyFilterSize)

% *************************************************************************
% FILTER_LAWS_3D: convolves an image with one, or many 3D Laws kernel(s)
% *************************************************************************
%
% INPUTS
%
...
%
%   lawsKernel, a cell array of 3D Laws kernels
%
...
h1 = imfilter(image,lawsKernel{1,1},padArg,'conv');
...
if ~invariant
    ...
    for i = 2:length(lawsKernel)
        h(:,:,:,i) = imfilter(image,lawsKernel{i,1},padArg,'conv');
    end
    ...
    imageFiltered{3,1} = filter_mean_3D(abs(hPooled),...
        ones(1,3).*((2*energyFilterSize)+1),padType);
